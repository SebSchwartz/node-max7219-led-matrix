...
max7219.prototype._init = function(){
    if(typeof this._spi !=='undefined' && !init){
        this._write(MAX7219_REG_SCANLIMIT, 7);  // show all 8 digits
        this._write(MAX7219_REG_DECODEMODE, 0x0); // use matrix (not digits)
        this._write(MAX7219_REG_DISPLAYTEST, 0x0); // no display test
        this._write(MAX7219_REG_SHUTDOWN, 0x1); // not shutdown mode
...
max7219.prototype._write = function(register, data){
    var buf = new Buffer([register, data]);
    this._spi.write(buf, function(device, buf2) {
        var s = "";
...
max7219.prototype.setBrightness = function(intensity){
    if(intensity<0 && intensity>15){
        throw 'Invalid brightness: '+intensity;
    }
    this._write(MAX7219_REG_INTENSITY, intensity);
}

max7219.prototype.clear = function(){
    for(var i; i<NUM_DIGITS; i++){
        this._write(MAX7219_REG_DIGIT0 + i, 0x0);
    }

}

max7219.prototype.letter = function(letter, font){
...
max7219.prototype._ascii = function(a) {
    return a.charCodeAt(0); 
}

max7219.prototype.showMessage = function(text, font){
    if(typeof font === 'undefined'){
        font = DEFAULT_FONT;
    }
